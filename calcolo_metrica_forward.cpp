            ...
            if(depth - current_depth > 0) {
                ...
                if(this->size > 1) {
                    ...
                    if(num_right == this->size || num_left == this->size) {
                        ...
                        auto* inds_tmp = new int[this->size]; //ok
                        for (int i = 0; i < this->size; i++) {
...
int main() {
    ...
    for (int h = 0; h < NUM_TRIANGLES; h++) {
        ...
        for (int w = 0; w < NUM_DIMENSIONS; w++) {
            tmp1[w] = verts[triangles[h][1]][w] - verts[triangles[h][0]][w];
            tmp2[w] = verts[triangles[h][2]][w] - verts[triangles[h][0]][w];
        }
        ...
        tr_areas[h] = sqrt(tr_areas[h])/2;

        tr_areas_sum += tr_areas[h];
    }
    
    int*  centr_generator_range = new int[NUM_TRIANGLES];
    centr_generator_range[0] = static_cast<int>(ceil(exp(log(tr_areas[0]) - log(tr_areas_sum) + log(NUM_POINTS_OVERSAMP))));

    for (int i = 1; i < NUM_TRIANGLES; i++) {
        centr_generator_range[i] = centr_generator_range[i - 1] +
            static_cast<int>(ceil(exp(log(tr_areas[i]) - log(tr_areas_sum) + log(NUM_POINTS_OVERSAMP))));
    }
    ...
    for (int part_ind = 1; part_ind <= NUM_PARTS*NUM_SIMS; part_ind++) {
        ...
        auto* my_tree = new kd_tree(verts_scan, triangles_scan, 15, indices, NUM_TRIANGLES_SCAN);
        ...
        for(int b = 0; b < NUM_POINTS_OVERSAMP; b++) {
            ...
            sp[0] = verts[triangles[i_tr][0]][0] +
                (verts[triangles[i_tr][1]][0] - verts[triangles[i_tr][0]][0]) * tmp4 +
                    (verts[triangles[i_tr][2]][0] - verts[triangles[i_tr][0]][0]) * tmp5;
            sp[1] = verts[triangles[i_tr][0]][1] +
                (verts[triangles[i_tr][1]][1] - verts[triangles[i_tr][0]][1]) * tmp4 +
                    (verts[triangles[i_tr][2]][1] - verts[triangles[i_tr][0]][1]) * tmp5;
            sp[2] = verts[triangles[i_tr][0]][2] +
            ...
            bool already_checked[NUM_TRIANGLES_SCAN];
            for (int ac = 0; ac < NUM_TRIANGLES_SCAN; ac++) {
